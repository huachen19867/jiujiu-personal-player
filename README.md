...
Android App 内选歌与播放已经优先走原生能力：多选使用 `ACTION_OPEN_DOCUMENT`，播放使用 Android `MediaPlayer`，并接入 `MediaSession` 媒体通知与 `mediaPlayback` 前台服务。当前前端会把播放范围里的原生音频队列同步给 Android 层，因此后台播完当前曲目、或锁屏点下一首时，原生层可以直接加载下一首并继续播放。若应用被系统强杀、从任务列表划掉，或被厂商省电策略强限制，后台播放仍可能停止；正常锁屏/切后台播放已优先走原生前台播放链路。
