...
- 本地音乐播放：支持从手机本地选择音频文件，也支持选择一个文件夹并递归导入其中的音频。
...
1. 安装 APK 后打开 App。
2. 点歌单卡片上的加号添加歌曲；可以选单首/多首，也可以选整个文件夹。如果本地歌曲特别多，优先进入“自动读取本地”歌单读取媒体库。
3. 点歌曲左侧播放按钮开始播放。
...
如果歌曲已经整理在某个文件夹里，可以用歌单加号里的“选文件夹”。App 会递归扫描这个文件夹和它下面的小文件夹，把找到的音频全部加入当前歌单；子文件夹不会自动拆成新歌单，避免导入结果和用户当前选中的歌单目标冲突。
...
- `C:\AI\Android\jiujiu-personal-player-v1.0.22-debug.apk`
